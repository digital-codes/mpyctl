...
if ($type == "up"){
  ...
  if ($resultWithCrc->Crc == $inputCrc) {
    ...
    file_put_contents("/var/www/html/cs/mrsplz.log", date("Y-m-d H:i:s") . " Config OK" . PHP_EOL, FILE_APPEND);
    ...
    file_put_contents("/var/www/html/cs/mrsplz.log", date("Y-m-d H:i:s") . " DB:" . $dbhost . ", " . $dbname . " ," .  $dbuser . ", " . $dbpwd . $PHP_EOL, FILE_APPEND);

    // prepare the row
    $row = unpackPayload($dataRaw);
    file_put_contents("/var/www/html/cs/mrsplz.log", date("Y-m-d H:i:s") . " Converted" . json_encode($row) . PHP_EOL, FILE_APPEND);
    // skip id = 0, used during join sequence
    if ($row["id"] != 0){  
      $result = insertRow($dbhost, $dbname, $dbuser, $dbpwd,$row);
      file_put_contents("/var/www/html/cs/mrsplz.log", date("Y-m-d H:i:s") . " Inserted: " . $result . PHP_EOL, FILE_APPEND);
    }
  } else {
