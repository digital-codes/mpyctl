...
// copied from https://github.com/meetanthony/crcphp/tree/master
...
function testCrc() {
    ...
    global $CRC_16_X_25_;
    $crc16 = new Crc16();
    $data = array(0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39); // ASCII for "123456789"
    $result = $crc16->ComputeCrc($CRC_16_X_25_, $data);
    echo "Test CRC-16/X-25: " . dechex($result->Crc) . "\n"; // Output: CRC-16/X-25: 906e

    // real example usage
    echo "Real example usage:\n";
    $hexDataWithCrc = "0B020000000500C0012A0033004703E90010000F0019CC52";
    echo "Hex data with CRC: " . $hexDataWithCrc . "\n";
    // Convert hex string to byte array. do not include last 2 bytes (the CRC itself) in the data for computing the CRC
    $dataWithCrc = array_map('hexdec', str_split($hexDataWithCrc, 2));
    $data = array_slice($dataWithCrc, 0, -2); // exclude last 2 bytes (the CRC itself) for computing the CRC
    $resultWithCrc = $crc16->ComputeCrc($CRC_16_X_25_, $data);
    echo "CRC-16/X-25 for data with CRC: " . dechex($resultWithCrc->Crc) . "\n"; // Output: CRC-16/X-25 for data with CRC: 906e 
    // input crc is last 2 bytes of the data, so we can check if the computed CRC matches the input CRC 
    $inputCrc = ($dataWithCrc[count($dataWithCrc) - 2] << 8) | $dataWithCrc[count($dataWithCrc) - 1];
    if ($resultWithCrc->Crc == $inputCrc) {
        echo "CRC matches input CRC.\n";
    } else {
        echo "CRC does not match input CRC.\n";
    }   
}
