...
##### add motor config here
try: 
    print("Motor pins:",len(cfdata["io"]["motor"]))
except:
    raise BaseException("Invalid Config")        

##### init motor outputs
motor = []
for p in cfdata["io"]["motor"]:
    motor.append({
        "io":p,
...
##### globals for motor
_motorSteps = const(10)
_numMotors = (int(len(motor) // 2))
...
def motorDrive(midx,value,brk=True):
    ...
    if value == 0:
        ...
        setPwm(midx*2+1)
        duty = int(abs(value) / _motorSteps * 1023)
        print("Rev duty:",duty,brk)
        motor[midx*2+1]["pin"].duty(duty)
    else:
        # forward
        if abs(value) > _motorSteps:
            value = _motorSteps
        setPwm(midx*2)
        duty = int(abs(value) / _motorSteps * 1023)
        print("Fwd duty:",duty,brk)
        motor[midx*2]["pin"].duty(duty)
        if brk:
            setPwm(midx*2+1)
            motor[midx*2+1]["pin"].duty(0)
        else:
...
# this is a rover car
cfg_value = bytearray([0x55,0xaa,3,0])
    ...
    while True:
        if connected and authorized:
            try:
                ...
                print("received:",_value)
                data = decryptMsgWithIv(_value)
                # expect data like speed -10..10 per motor (4 bytes), 1 byte BRK, 2 controls for on/off
                # like led or grip
                print("received data:",data)
                if len(data) != 7:
                    print("Invalid data length:",data)
                    #raise BaseException("Bad CTL")
                    data = struct.pack("<bbbbbbb",-4,0,3,0,1,1,0)
                else:
                    print("Ctl ok")
                ctlData = struct.unpack("<bbbbbbb", data)
                stopEngine = ctlData[4]
                for i in range(_numMotors):
                    motorDrive(i,ctlData[i],stopEngine)
                if ctlData[5] == 0:
                    rgbFill((100,0,0))
                else:
                    rgbFill((0,100,0))
